...
| **バージョン表示 (`--version`)**    | `MyNKF --version` または `MyNKF -v` | バージョン番号 `MyNKF v1.6.0` が出力されて正常終了すること。                 | **合格** |
| **バージョン別名表示 (`--versio`)** | `MyNKF --versio`                    | バージョン番号 `MyNKF v1.6.0` が出力されて正常終了すること。                 | **合格** |
...
コアロジックの正確性を保証するため、`src/tests.rs` に 21 個のテストケースからなるテストモジュールを実装し、すべて合格しました。
...
- **`test_guess_encoding_binary_with_null`**: テキスト中にヌルバイトが含まれる場合に `BINARY` (Unknown) と判定すること。(合格)
...
- **`test_kanji_roundtrip`**: 日本語漢字・ひらがな・カタカナ・記号の UTF-8 ⇆ Shift_JIS ラウンドトリップ変換が文字化けなく行われること。(合格)
- **`test_eucjp_roundtrip`**: 日本語漢字・ひらがな・カタカナ・記号の UTF-8 ⇆ EUC-JP ラウンドトリップ変換が文字化けなく行われること。(合格)
...
- **`test_convert_line_endings_raw`**: 再エンコードを行わずに CRLF/LF/CR の改行コード置換が安全に行われること。(合格)
...
```text
running 21 tests
test tests::test_convert_line_endings_raw ... ok
...
test tests::test_guess_encoding_binary_with_null ... ok
...
test tests::test_eucjp_roundtrip ... ok
test tests::test_kanji_roundtrip ... ok
...
GitHub Actions の動作安定化、およびローカル環境でのビルド・品質検証結果です。

### 5.1 ローカル品質検証

ローカル環境にて以下の検証コマンドを実行し、すべての項目に合格しました。

| 検証項目                   | 実行コマンド                                   | 結果                                      | 合否     |
| :------------------------- | :--------------------------------------------- | :---------------------------------------- | :------- |
| **コードフォーマット検証** | `cargo fmt --check`                            | 警告や差分なく正常終了                    | **合格** |
| **静的解析 (Clippy)**      | `cargo clippy --all-targets -- -D warnings`    | 警告・エラーの発生なく正常終了            | **合格** |
| **自動単体テスト**         | `cargo test`                                   | 21件すべてのテストが正常に通過 (0 failed) | **合格** |
| **ドキュメントビルド検証** | `cargo doc --no-deps --document-private-items` | 警告・エラーなく正常終了                  | **合格** |
